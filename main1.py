...
from architecture.ips_net10 import IPSNet
from training.iterative3 import train_one_epoch, evaluate
import random
os.environ["WANDB_MODE"] = "offline"
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(device)
dataset = 'cqu_bpdd' # either one of {'mnist', 'camelyon', 'traffic','cqu_bpdd','fmow'}


# get config
with open(os.path.join('config', dataset + '_config2.yml'), "r") as ymlfile:
...
if dataset == 'mnist':
    ...
    #val_data = MyDataset(conf, _type='val')
...
for epoch in range(conf.n_epoch):
    ...
    acc = log_writer_test.metrics['sign'][epoch]
    f1 = log_writer_test.f1
...
if conf.wandb:
    wandb.log({
            'test_acc1': log_writer_test.metrics['sign'][-1],

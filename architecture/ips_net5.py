...
from architecture.transformer5 import Transformer, pos_enc_1d
...
class IPSNet(nn.Module):
    ...
    def score_and_select(self, emb, emb_pos, M, idx, temp_token):
        """ 
        ...
        attn,temp_token = self.transf.get_scores_and_tokens(emb_to_score,temp_token) # (B, M+I)
        ...
        return mem_emb, mem_idx, temp_token
    ...
    def ips(self, patches):
        ...
        temp_token = self.transf.crs_attn.q
        global_token_list=[]
        ...
        for i in range(n_iter):
            ...
            mem_emb, mem_idx, temp_token = self.score_and_select(all_emb, all_emb_pos, M, all_idx, temp_token)
            global_token_list.append(temp_token)
            temp_token=self.transf.crs_attn.q
            # print(temp_token.shape)
        ...
        global_token=torch.cat(global_token_list,dim=1)
        return mem_patch, mem_pos, global_token

    def forward(self, mem_patch, mem_pos=None, global_token=None):
        """
        ...
        mem_emb=torch.cat([mem_emb,global_token],dim=1)

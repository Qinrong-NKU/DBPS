...
class IPSNet(nn.Module):
    ...
    def ips(self, patches):
        ...
        ## Embed
        all_emb = self.encoder(init_patch.reshape(-1, *patch_shape[2:]))
        all_emb = all_emb.view(B, M, -1)
        ...
        for i in range(n_iter):
            ...
            all_emb = torch.cat((all_emb, iter_emb), dim=1)
            all_idx = torch.cat((all_idx, iter_idx), dim=1)
        ...
        # Select patches
        mem_emb, mem_idx = self.score_and_select(all_emb, all_emb_pos, M, all_idx)
        whole_emb, large_idx = self.score_and_select(all_emb, all_emb_pos, N, all_idx)

...
class MultiHeadCrossAttention(nn.Module):
    ...
    def get_attn_and_token(self, x, token):
        D_k, D_v, H, n_token = self.D_k, self.D_v, self.H, self.n_token
        B, len_seq = x.shape[:2]

        # project and separate heads
        q = self.q_w(token).view(token.shape[0], n_token, H, D_k)
        k = self.k_w(x).view(B, len_seq, H, D_k)
...
class Transformer(nn.Module):
    ...
    def get_scores_and_tokens(self, x, token):

        token,attn = self.crs_attn.get_attn_and_token(x,token)
        # Average scores over heads and tasks
        # Average over tasks is only required for multi-task learning (mnist).
        return attn.mean(dim=1).transpose(1, 2).mean(-1),token

...
from architecture.transformer import Transformer, Transformer6, pos_enc_1d
import time
...
class IPSNet(nn.Module):
    ...
    def __init__(self, device, conf):
        ...
        self.transf2 = Transformer6(12, conf.H, conf.D, conf.D_k, conf.D_v,
            conf.D_inner, conf.attn_dropout, conf.dropout)
    ...
    def ips(self, patches):
        ...
        for i in range(n_iter):
            ...
            # time1 = time.perf_counter()
            mem_emb, mem_idx = self.score_and_select(all_emb, all_emb_pos, M+S, all_idx)
            # time2 = time.perf_counter()
            # print('程序transformer运行时间:%s毫秒' % ((time2 - time1) * 1000))
            second_emb_list.append(mem_emb[:, M:])
            second_idx_list.append(mem_idx[:, M:])
            mem_emb, mem_idx = mem_emb[:, :M], mem_idx[:, :M]
        # Select patches
        second_emb=torch.cat(second_emb_list,dim=1)
        second_idx=torch.cat(second_idx_list,dim=1)
        second_emb_pos=None
        whole_emb, large_idx = self.score_and_select(second_emb, second_emb_pos, second_emb.shape[1], second_idx)
    ...
    def forward(self, mem_patch, mem_pos=None,whole_emb=None,mem_idx=None,large_idx=None):
        ...
        whole_emb=self.transf2(whole_emb)
        whole_emb=torch.cat([mem_emb,whole_emb],dim=1)
        if torch.is_tensor(mem_pos):
            mem_emb = mem_emb + mem_pos

        image_emb = self.transf(whole_emb)

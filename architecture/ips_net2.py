...
class IPSNet(nn.Module):
    ...
    def ips(self, patches):
        ...
        if self.training:
            self.encoder.train()
            self.transf.train()
        whole_emb=torch.cat(whole_emb,dim=1)
        # Return selected patch and corresponding positional embeddings
        return mem_patch, mem_pos,whole_emb,mem_idx

    def forward(self, mem_patch, mem_pos=None,whole_emb=None,mem_idx=None):
        """
        ...
        for i in range(mem_idx.shape[0]):
            for j in range(mem_idx.shape[1]):
                whole_emb[i][mem_idx[i][j]]=mem_emb[i][j]

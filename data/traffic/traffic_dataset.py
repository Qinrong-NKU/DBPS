...
class TrafficSigns(Dataset):
    ...
    def __init__(self, conf, train=True):
        ...
        # if train:
        #     transform_list +=[
        #         transforms.RandomErasing(scale=(0.9,0.91),p=0.9)
        #     ]
    ...
    def __getitem__(self, i):
        ...
        # if self.train:
        #     ids=random.sample(self.list,self.N-6*(self.I-int(self.I*0.6)))
        #     patches=patches[ids]


        if self.train:
            ids=random.sample(self.list,int(self.N*0.1))
            patches[ids]=self.transform1(patches[ids])
